...
/// ASCII character.
///
/// If `s` has the proper form, return `Some<(x, y)>`. If it can't be parsed,
/// return `None`.
...
/// Determine whether `c` is in the Mandelbrot set, using at most `limit`
/// iterations.
///
/// If `c` is not a member, return `Some(i)`, where `i` is the number of
/// iterations it took for `c` to leave the circle of redius 2 centered on the
/// origin. If `c` seems to be a member (i.e., we reached the iteration limit
/// without proving that `c` is not a member), return `None`.
